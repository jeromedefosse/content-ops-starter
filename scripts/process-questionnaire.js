...
function main() {
  ...
  const historyFile = path.join(patientDir, 'history.json');
  let history = [];
  ...
  const resultJson = path.join(patientDir, 'result.json');
  fs.writeFileSync(resultJson, JSON.stringify(result, null, 2) + '\n');

  const resultPdf = path.join(patientDir, 'result.pdf');
  generatePdf(result, resultPdf);

  const archiveDir = path.join(patientDir, 'archive');
